...
- **LONG FULL-SUITE RUNS: WRITE THE FILE FIRST, PASS AN EXPLICIT TIMEOUT.** Every prompt that asks an agent to run the full suite must include: "Create your output file with your findings so far BEFORE starting the full-suite run, then append the results. Pass an explicit long timeout (e.g. `timeout: 600000`) to the test command — do not rely on the default."
  **Why this rule exists (Sept 2026 chat-push-json-body run):** the full suite here is ~3141 Docker-run tests; an agent stalled mid-run and lost everything it had already verified, forcing a full re-run of that gate (`## Pipeline Results`: "post-implementation audit | 1 (+1 stall retry)"). A file written early survives a stall; an in-memory review does not.
