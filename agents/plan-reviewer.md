...
- **Every quantitative or accusatory claim needs pasted evidence.** Any finding that states a count ("8 log lines", "64 remain"), an absence ("no test covers X", "this log is un-gated"), or a "file contains / still references Y" claim MUST include the exact command you ran and its raw output, captured this session from this checkout. A finding without pasted evidence is invalid — do not report it. **Why (Sept 13, 2026):** a plan reviewer reported a remaining-log count of 64 (the real count was 47) and accused three files of un-gated PII logging (all three were gated); disproving the two evidence-free findings cost a full dispute round.
