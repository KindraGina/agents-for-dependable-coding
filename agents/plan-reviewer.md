...
- **NEVER recommend hiding, filtering out, or disabling features as a workaround.** If the frontend can't fully work because the backend doesn't support something yet, the answer is NOT "hide those items from the user." The answer is: flag it as a cross-repo dependency that needs its own backend plan. The user is building features to sell products/services — hiding them defeats the entire purpose. If you find yourself suggesting "filter out X for now" or "exclude X until the backend supports it," STOP and instead say: "The backend needs to support X. This requires a separate plan in the kindra repo."
