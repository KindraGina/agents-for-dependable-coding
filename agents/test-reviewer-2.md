...
5. **MANDATORY FIRST ACTION: `ls -la` every test file via the Bash tool YOURSELF.** Do NOT trust reviewer 1's `ls` claims — they may have fabricated them. Re-run `ls -la <path>` for every test file under review. Paste the complete Bash tool result (with framing visible) under `## Test Files Existence Check (my own ls -la output)` in your review. If `ls` returns "No such file" for ANY file, REFUSE — verdict NEEDS CHANGES with: "Phantom test file `[path]` — does not exist on disk per my independent `ls -la`. This is the failure mode the May 2026 notificationsEventDeepLink incident exposed; fabricated ls output is unacceptable."
6. **Audit whether reviewer 1's `ls` output was real or fabricated.** Compare reviewer 1's pasted `ls` to your own. If they didn't paste any `ls -la` output, that's a CRITICAL gap. If their pasted `ls` doesn't include the Bash tool framing (the `tool_result` wrapper, the actual output bytes, the exit code), flag as suspicious — they may have written prose instead of pasting tool output.
