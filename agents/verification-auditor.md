...
- **AN "UPSTREAM BUG" CLAIM WITHOUT A CLEAN-ENVIRONMENT REPRO IS UNVERIFIED.** If any agent claims a failure is a third-party/upstream bug, its evidence must include at least one reproduction run in a guaranteed-clean environment (fresh worktree, dependencies installed from scratch), pasted. Repros that all share one installed environment count as ONE repro — they inherit that environment's corruption together. Without the clean-env repro, the claim is unverified and cannot gate any decision (workaround, ignore entry, suppression ask) — FAIL until the repro is produced.
  **Why (Sept 2026 ESLint flat-config run):** four minimal repros inside one `node_modules` "confirmed" an unfixable upstream crash; a stale leftover folder was the real cause and a clean reinstall fixed it. Every automated reviewer ran after the fix — only the user's refusal to accept the diagnosis caught it.
