...
- **PROVE THE PROOF TEST MECHANICALLY, DON'T REASON ABOUT IT.** For the single test that most directly proves the fix: `git stash push -- <changed source files>` (source only, never tests), run that test file, paste the RED output, `git stash pop`, re-run, paste the GREEN output, and confirm `git status` is back to its prior state. If it passes RED, the test proves nothing — NEEDS CHANGES.
